...
vi.mock('./config.js', async () => {
  ...
  return { ...actual, DATA_DIR: '/tmp/nanoclawd-test-host' };
});
...
const TEST_DIR = '/tmp/nanoclawd-test-host';
...
describe('session manager', () => {
  ...
  it('should refuse path-traversal in attachment filenames', () => {
    ...
    const escapeTarget = path.join('/tmp', 'nanoclawd-traversal-canary');
    ...
    writeSessionMessage('ag-1', session.id, {
      ...
      content: JSON.stringify({
        ...
        attachments: [
          {
            type: 'document',
            name: '../../../../../../../../tmp/nanoclawd-traversal-canary',
