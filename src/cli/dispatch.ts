...
export async function dispatch(req: RequestFrame, ctx: CallerContext): Promise<ResponseFrame> {
  ...
  // all positional args with dashes (e.g. `nclwd groups get abc123`
  ...
  if (ctx.caller !== 'host' && cmd.access === 'approval') {
    ...
    await requestApproval({
      ...
      question: `Agent "${agentName}" wants to run:\n\`nclwd ${req.command}${argSummary ? ' ' + argSummary : ''}\``,
    });
...
registerApprovalHandler('cli_command', async ({ session, payload, userId, notify }) => {
  ...
  if (response.ok) {
    ...
    notify(`Your \`nclwd ${frame.command}\` request was approved and executed.\n\n${data}`);
  } else {
    notify(`Your \`nclwd ${frame.command}\` request was approved but failed: ${response.error.message}`);
  }
});
