/**
 * Init the first (or Nth) NanoClawd v2 agent for a DM channel.
 *
...
async function main(): Promise<void> {
  ...
  initGroupFilesystem(ag, {
    instructions:
      `# ${args.agentName}\n\n` +
      `You are ${args.agentName}, a personal NanoClawd agent for ${args.displayName}. ` +
...
async function sendWelcomeViaCliSocket(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    socket.once('error', (err) =>
      settle(
        new Error(
          `CLI socket at ${sockPath} not reachable: ${err.message}. Is the NanoClawd service running?`,
        ),
      ),
    );
    socket.once('connect', () => {
