/**
 * nclwd — chat with your NanoClawd agent from the terminal.
 *
 * Usage:
 ...
 *
 * Preconditions: NanoClawd host service running, an agent group wired to
...
function main(): void {
  ...
  socket.on('error', (err) => {
    ...
    if (e.code === 'ENOENT' || e.code === 'ECONNREFUSED') {
      console.error(`NanoClawd daemon not reachable at ${socketPath()}.`);
      console.error('Start the service (launchctl/systemd) before running nclwd.');
    } else {
