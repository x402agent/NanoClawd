...
// Deferred SDK builtins that either sidestep nanoclawd's own scheduling or
// don't fit our async message-passing model (they're designed for Claude
// Code's interactive UI and would hang here).
//
// - CronCreate / CronDelete / CronList / ScheduleWakeup: we have durable
//   scheduling via mcp__nanoclawd__schedule_task.
// - AskUserQuestion: SDK returns a placeholder instead of blocking on a
//   real answer — we have mcp__nanoclawd__ask_user_question that persists
...
// Tool allowlist for NanoClawd agent containers. MCP-tool entries are derived
...
const preToolUseHook: HookCallback = async (input) => {
  ...
  if (SDK_DISALLOWED_TOOLS.includes(toolName)) {
    return {
      decision: 'block',
      stopReason: `Tool '${toolName}' is not available in this environment — use the nanoclawd equivalent.`,
