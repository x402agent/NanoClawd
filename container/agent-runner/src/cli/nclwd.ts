#!/usr/bin/env bun
/**
 * nclwd — NanoClawd CLI client (container edition).
 *
 * Same interface as the host-side `bin/nclwd`. Detects that it's inside a
...
function printUsage(): void {
  process.stdout.write(
    ['Usage: nclwd <command> [--key value ...] [--json]', '', 'Run `nclwd help` to list available commands.', ''].join('\n'),
  );
}
...
if (!resp) {
  process.stderr.write('nclwd: command timed out after 30s\n');
  process.exit(2);
}
