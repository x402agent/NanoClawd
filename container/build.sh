#!/bin/bash
# Build the NanoClawd agent container image.
#
...
# Derive the image name from the project root so two NanoClawd installs on the
# same host don't overwrite each other's `nanoclawd-agent:latest` tag. Matches
...
echo "Building NanoClawd agent container image..."
