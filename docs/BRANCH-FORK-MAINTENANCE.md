...
**`qwibitai/nanoclawd`** (upstream) — core engine with skill definitions (`.claude/skills/`). No channel code on `main`.

**Channel forks** (`nanoclawd-whatsapp`, `nanoclawd-telegram`, `nanoclawd-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges nanoclawd-whatsapp fork  → adds WhatsApp
...
```bash
# In your local nanoclawd checkout
git checkout main && git pull

# For a fork:
git fetch nanoclawd-whatsapp
git checkout -B whatsapp-merge nanoclawd-whatsapp/main
git merge main
...
git push nanoclawd-whatsapp HEAD:main
...
1. Fork `nanoclawd` to `nanoclawd-{channel}`
