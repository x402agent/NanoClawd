...
- A bridge wraps the Chat instance + adapter to conform to NanoClawd's standard channel interface
- Chat SDK handles: webhook parsing, dedup, message history, platform API calls, rich content delivery
- NanoClawd handles: routing, agent lifecycle, session management

**Chat SDK's subscription model:**

Chat SDK has its own thread-level subscription concept (distinct from NanoClawd's channel-level registration):
...
This is sub-channel granularity. NanoClawd registers at the channel level ("listen to this Discord channel"). Chat SDK subscribes at the thread level ("track this specific Slack thread"). The bridge lets Chat SDK manage its own subscriptions internally — NanoClawd doesn't interfere with or replicate this.
...
Non-Chat-SDK channels (WhatsApp via Baileys, Gmail, custom integrations) implement the NanoClawd channel interface directly — no bridge, no Chat SDK types.
...
**`chat`** — simple NanoClawd format. Any channel can produce this.
```json
{
  "sender": "John",
  "senderId": "user123",
...
**`chat`** — simple NanoClawd format. NanoClawd channel delivers via `sendMessage(text)`.
```json
...
NanoClawd does not stream tokens to users. The Claude Agent SDK's `query()` yields complete results. The agent-runner writes one complete message to messages_out per result. The host delivers complete messages to channels.
...
NanoClawd is customized via skills — branches that get merged into the user's installation. Different skills add different capabilities (channels, integrations, behaviors). The code must be structured so that:
...
- Commands that don't make sense in the NanoClawd context or could cause issues
...
- **[api-details.md](api-details.md)** — Channel adapter interface (NanoClawd + Chat SDK bridge), message content examples, host delivery logic
