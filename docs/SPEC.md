...
│  │    • mcp__nanoclawd__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── nanoclawd.log               # Host stdout
│   └── nanoclawd.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nanoclawd.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nanoclawd-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nanoclawd installation
...
NanoClawd uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: nanoclawd (scheduler)
   │
   ▼
...
NanoClawd has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__nanoclawd__schedule_task]
        {
...
### NanoClawd MCP (built-in)

The `nanoclawd` MCP server is created dynamically per agent call with the current group's context.
...
NanoClawd runs as a single macOS launchd service.

### Startup Sequence

When NanoClawd starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NanoClawd containers from previous runs
...
### Service: com.nanoclawd

**launchd/com.nanoclawd.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/nanoclawd.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanoclawd.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nanoclawd.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep nanoclawd

# View logs
tail -f logs/nanoclawd.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanoclawd` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NanoClawd auto-starts container runtime but may fail |
...
- `logs/nanoclawd.log` - stdout
- `logs/nanoclawd.error.log` - stderr
