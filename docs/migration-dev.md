...
1. Check both services aren't running: `systemctl --user list-units 'nanoclawd*'`
2. Check error log: `tail logs/nanoclawd.error.log`
...
1. The v2 service name is `nanoclawd-v2-<hash>` — find it: `systemctl --user list-units 'nanoclawd*'`
2. Manually stop: `systemctl --user stop <unit> && systemctl --user disable <unit>`
3. Restart v1: `systemctl --user start nanoclawd`
