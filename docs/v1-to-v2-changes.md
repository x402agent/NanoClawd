# NanoClawd v1 → v2 — what changed

Big-picture differences between NanoClawd v1 (the `~/nanoclawd` checkout you've been running) and v2 (this rewrite). Not a migration guide — that's what `bash migrate-v2.sh` and the `/migrate-from-v1` skill are for. This doc is the **vocabulary**: when something has moved or been renamed, find it here.
