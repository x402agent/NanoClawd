...
State backing the `SqliteStateAdapter` used by the Chat SDK bridge (see [api-details.md](api-details.md)). NanoClawd code rarely touches these directly — they're owned by `src/state-sqlite.ts`.
