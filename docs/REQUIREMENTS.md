...
This is a lightweight, secure alternative to OpenClawd (formerly ClawBot). That project became a monstrosity - 4-5 different processes running different gateways, endless configuration files, endless integrations. It's a security nightmare where agents don't run in isolated processes; there's all kinds of leaky workarounds trying to prevent them from accessing parts of the system they shouldn't. It's impossible for anyone to realistically understand the whole codebase. When you run it you're kind of just yoloing it.

NanoClawd gives you the core functionality without that mess.
...
> **Note:** Telegram, Slack, Discord, Gmail, and Apple Container skills already exist. See the [skills documentation](https://docs.nanoclawd.dev/integrations/skills-system) for the full list.
...
- Custom `nanoclawd` MCP server (inside container) provides scheduling tools
...
- `/update-nanoclawd` - Pull upstream changes, merge with customizations
...
**NanoClawd** - A reference to Clawdbot (now OpenClawd).
