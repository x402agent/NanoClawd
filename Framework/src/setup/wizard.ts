...
export async function runSpawnWizard(input: WizardInput): Promise<WizardOutput> {
  // The DB row is the source of truth for "fully spawned." A keystore on its
  // own means a previous --spawn generated a keypair but the on-chain mint
  // never completed (e.g. unfunded wallet). In that case, reuse the existing
  // keypair so the user can fund it and retry without losing the address.
  const existing = getLeviathan();
  if (existing) {
    throw new Error(
      `A leviathan already exists at ${SHELL_DIR} (pubkey ${existing.pubkey}). Each leviathan only spawns once. To create another, run on a different host or use --spawn-spawnling.`
    );
  }
