...
const MIN_SPAWN_LAMPORTS = 10_000_000; // 0.01 SOL — covers rent + tx fees for mintAndSubmitAgent
...
export async function spawnOnchain(input: SpawnOnchainInput): Promise<SpawnOnchainResult> {
  ...
  // Metaplex's Agent Registry API rejects unfunded wallets with an opaque
  // "AccountNotFound" error. Pre-check the balance so the user gets a clear
  // instruction instead of a stack trace from inside mpl-agent-registry.
  ...
  if (lamports < MIN_SPAWN_LAMPORTS) {
    ...
    throw new Error(
      `Leviathan wallet ${pubkey} is unfunded on ${net} (have ${have} SOL, need at least ${need} SOL). ` +
      `Send SOL to that address, then re-run --spawn. ` +
      `On devnet, run: solana airdrop 1 ${pubkey} --url devnet`
    );
  }
