...
# never fail the caller. Honors NANOCLAWD_NO_DIAGNOSTICS=1. The distinct_id
...
NANOCLAWD_PH_KEY='phc_fx1Hhx9ucz8GuaJC8LVZWO8u03yXZZJJ6ObS4yplnaP'
NANOCLAWD_PH_URL='https://us.i.posthog.com/capture/'
...
ph_event() {
  [ "${NANOCLAWD_NO_DIAGNOSTICS:-}" = "1" ] && return 0
  local event=$1
  shift
  local id
  id=$(ph_install_id)
  ...
  payload=$(printf '{"api_key":"%s","event":"%s","distinct_id":"%s","properties":{%s}}' \
    "$NANOCLAWD_PH_KEY" "$event" "$id" "$props")

  curl -sS --max-time 3 -X POST "$NANOCLAWD_PH_URL" \
