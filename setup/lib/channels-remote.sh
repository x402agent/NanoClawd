...
#
# Typical fork setups keep the upstream nanoclawd repo under a remote named
...
# at qwibitai/nanoclawd, and prints its name.
#
...
resolve_channels_remote() {
  ...
  # remote-add failure.
  git remote add upstream https://github.com/qwibitai/nanoclawd.git 2>/dev/null || true
  printf '%s' "upstream"
}
