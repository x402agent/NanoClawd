 ...
 */
export const HELP_ESCAPE_SENTINEL = '__NANOCLAWD_HELP_ESCAPE__';
...
function buildSystemPrompt(ctx: HandoffContext): string {
  const lines: string[] = [
    `The user is running NanoClawd's interactive \`setup:auto\` flow to wire the ${ctx.channel} channel.`,
...
/**
 * Dispatcher: checks NANOCLAWD_SETUP_ASSIST_MODE and delegates to either
...
export async function offerClaudeOnFailure(
  ...
  if (process.env.NANOCLAWD_SETUP_ASSIST_MODE === 'true' || process.env.NANOCLAWD_SETUP_ASSIST_MODE === '1') {
...
function buildFailureSystemPrompt(ctx: AssistContext, projectRoot: string): string {
  ...
  const lines: string[] = [
    "The user is running NanoClawd's interactive setup flow and hit a failure.",
    '',
