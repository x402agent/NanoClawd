...
export async function run(_args: string[]): Promise<void> {
  ...
  // `com.nanoclawd` plist) will keep trashing this install's containers on
  ...
  if (peerReport.unloaded.length > 0) {
    log.warn('Unloaded unhealthy peer NanoClawd services', {
...
/**
 * Symlink bin/nclwd into ~/.local/bin so `nclwd` is available from anywhere.
...
function installCliSymlink(projectRoot: string, homeDir: string): void {
  const source = path.join(projectRoot, 'bin', 'nclwd');
  const targetDir = path.join(homeDir, '.local', 'bin');
  const target = path.join(targetDir, 'nclwd');
  ...
  try {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) {
        ...
        log.warn('~/.local/bin/nclwd exists and is not a symlink — skipping', { target });
        return;
      }
    } catch (e) {
    ...
    log.info('Installed nclwd CLI symlink', { target, source });
  } catch (err) {
    log.warn('Could not install nclwd CLI symlink (non-fatal)', { err });
  }
}
...
function setupLaunchd(
  ...
  // Per-checkout service label so multiple NanoClawd installs can coexist
...
<dict>
    ...
    <string>${projectRoot}/logs/nanoclawd.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanoclawd.error.log</string>
</dict>
</plist>`;
...
/**
 * Kill any orphaned nanoclawd node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    log.info('Stopped any orphaned nanoclawd processes');
  } catch {
...
function setupSystemd(
  ...
  const unit = `[Unit]
Description=NanoClawd Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/nanoclawd.log
StandardError=append:${projectRoot}/logs/nanoclawd.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanoclawd processes to avoid channel connection conflicts
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanoclawd.sh');
  const pidFile = path.join(projectRoot, 'nanoclawd.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanoclawd.sh — Start NanoClawd without systemd',
    ...
    '    echo "Stopping existing NanoClawd (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoClawd..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanoclawd.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanoclawd.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoClawd started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanoclawd.log"`,
  ];
