 ...
 *     rawLog);
 *
 * nanoclawd.sh emits the bootstrap entry directly via a bash helper so
...
/** Wipe prior logs and write a header. Called once per fresh run (by nanoclawd.sh or as a fallback by auto.ts if invoked standalone). */
 ...
 * pre-existing 01-bootstrap.log (written by nanoclawd.sh before this module
