...
async function main(): Promise<void> {
        ...
        const choice = ensureAnswer(
          await brightSelect({
            ...
            options: [
              ...
              {
                value: 'fresh',
                label: 'Install a fresh instance for NanoClawd',
  ...
  if (!skip.has('service')) {
    const res = await runQuietStep('service', {
      running: 'Starting NanoClawd in the background…',
      done: 'NanoClawd is running.',
    });
    if (!res.ok) {
      await fail('service', "Couldn't start NanoClawd.", 'See logs/nanoclawd.error.log for details.');
    }
    if (res.terminal?.fields.DOCKER_GROUP_STALE === 'true') {
      p.log.warn(brandBody("NanoClawd's permissions need a tweak before it can reach Docker."));
      p.log.message(
        brandBody(
    ...
    if (!res.ok) {
      ...
      if (service === 'running_other_checkout') {
        ...
        notes.push(
          wrapForGutter(
            [
              '• Your NanoClawd service is running from a different folder on this machine.',
  ...
  const rows: [string, string][] = [
    ...
    ["See what's happening:", 'tail -f logs/nanoclawd.log'],
  ...
  note(
    wrapForGutter(
      "NanoClawd runs on this machine. It's only reachable while this computer is on and connected to the internet. For always-on availability, run it on a cloud VM — or keep this machine awake.",
      6,
    ),
    'Heads up',
  );
...
function renderPingFailureNote(result: PingResult): void {
  const body =
    result === 'socket_error'
      ? [
          wrapForGutter(
            "The NanoClawd service isn't listening on its local socket. Try restarting it, then chat with `pnpm run chat hi`:",
            6,
          ),
          '',
...
function sendChatMessage(message: string): Promise<void> {
  return new Promise((resolve) => {
    // `pnpm --silent` suppresses the `> nanoclawd@… chat` preamble so the
...
async function runAuthStep(): Promise<void> {
  ...
  if (method === 'skip') {
    ...
    p.log.warn(
      brandBody(
        'Claude sign-in skipped. Re-run setup or run `bash nanoclawd.sh` to finish later.',
      ),
    );
    return;
  }
...
/**
 * Bootstrap (nanoclawd.sh) normally initializes logs/setup.log and writes
