...
async function ensureSignalCli(): Promise<void> {
  ...
  note(
    [
      "NanoClawd talks to Signal through signal-cli, which isn't installed yet.",
...
async function restartService(): Promise<void> {
  const s = p.spinner();
  s.start('Restarting NanoClawd so it sees your Signal account…');
  const start = Date.now();
  ...
  try {
    ...
    s.stop(`NanoClawd restarted. ${k.dim(`(${fmtDuration(Date.now() - start)})`)}`);
...
async function resolveAgentName(): Promise<string> {
  const preset = process.env.NANOCLAWD_AGENT_NAME?.trim();
  if (preset) {
