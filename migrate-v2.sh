#!/usr/bin/env bash
#
# migrate-v2.sh — Migrate a NanoClawd v1 install into this v2 checkout.
#
...
echo
bold "NanoClawd v1 → v2 migration"
echo
echo
...
# the same lookup nanoclawd.sh does.
...
find_v1() {
  ...
  # Scan sibling directories for anything clawd-ish with a v1 DB
  local parent
...
# on disk so the user can rollback with: systemctl --user start nanoclawd
