...
| `DC_DISPLAY_NAME` | `NanoClawd` | Bot display name shown in DeltaChat |
...
```bash
grep "invite link" logs/nanoclawd.log | tail -1
...
```bash
grep "Channel credentials missing" logs/nanoclawd.log | grep deltachat
```
...
```bash
grep "DeltaChat" logs/nanoclawd.log | tail -20
```
...
1. Check the service is running and the adapter started: `grep "Channel adapter started.*deltachat" logs/nanoclawd.log`
2. Check connectivity: `grep "DeltaChat: IO started" logs/nanoclawd.log`
...
```bash
grep "DeltaChat" logs/nanoclawd.error.log | tail -20
```
