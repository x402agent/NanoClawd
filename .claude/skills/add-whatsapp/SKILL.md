...
NanoClawd doesn't ship channels in trunk. This skill copies the native WhatsApp (Baileys) adapter and its `whatsapp-auth` setup step in from the `channels` branch. No Chat SDK bridge.
...
2. Connected: `grep "Connected to WhatsApp" logs/nanoclawd.log | tail -1`
...
4. Service running: `systemctl --user status nanoclawd`

### "conflict" disconnection

Two instances connected with same credentials. Ensure only one NanoClawd process is running.
