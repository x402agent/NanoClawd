...
NanoClawd users fork the repo and customize it — changing config values, editing source files, modifying personas, adding skills. When upstream ships updates or refactors, `git merge` produces painful conflicts because the same core files were changed on both sides.
...
Check remotes with `git remote -v`. If `upstream` is missing, ask for the URL (default: `https://github.com/qwibitai/nanoclawd.git`), add it, then `git fetch upstream --prune`.
...
Check for existing guide: `.nanoclawd-migrations/guide.md` or `.nanoclawd-migrations/index.md`.

**Determine the tier based on the total diff from base:**

### Tier 1: Lightweight — suggest `/update-nanoclawd` instead
...
Tell the user the scope is small and suggest `/update-nanoclawd` might be simpler. Let them choose.
...
For Tier 1:
- **Use /update-nanoclawd** — simpler merge-based approach
...
> Explore this NanoClawd fork to identify all changes from the upstream base. Run these commands and report back:
>
...
**Storage:** `.nanoclawd-migrations/guide.md` for Tier 2. `.nanoclawd-migrations/` directory with `index.md` and section files for Tier 3.
...
1. Stop the service (do this directly):
   ```bash
   launchctl unload ~/Library/LaunchAgents/com.nanoclawd.plist 2>/dev/null || true
   ```
...
cp -r "$WORKTREE_PATH/.nanoclawd-migrations" /tmp/nanoclawd-migrations-backup 2>/dev/null || true
...
cp -r /tmp/nanoclawd-migrations-backup/* .nanoclawd-migrations/ 2>/dev/null || true
rm -rf /tmp/nanoclawd-migrations-backup
```
...
1. Use the Read tool to read `.claude/skills/migrate-nanoclawd/diagnostics.md`.
