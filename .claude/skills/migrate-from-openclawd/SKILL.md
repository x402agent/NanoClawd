---
name: migrate-from-openclawd
description: Migrate from OpenClawd to NanoClawd. Detects existing OpenClawd installation, extracts identity, channel credentials, scheduled tasks, and other config, then guides interactive migration. Triggers on "migrate from openclawd", "openclawd migration", "import from openclawd".
---

# Migrate from OpenClawd

Guide the user through migrating their OpenClawd installation to NanoClawd. This is a conversation, not a batch job. Read OpenClawd state, discuss it with the user, make judgment calls together about what to bring over and how.

**Principle:** Never silently copy data. Read it, explain it, discuss where it belongs in NanoClawd's architecture, show proposed changes before applying. Credentials must be masked when displayed (first 4 + `...` + last 4 characters). Make judgment calls about what's core vs. reference material.
...
- **Deferred / Not Applicable** — unsupported channels, discussed customizations, OpenClawd-only features
...
Run the discovery script to find and summarize the OpenClawd installation:

```bash
pnpm exec tsx ${CLAUDE_SKILL_DIR}/scripts/discover-openclawd.ts
```
...
**Sanity-check the output:** The discovery script detects known structures but can silently miss data if OpenClawd's format has changed. Check `CONFIG_TOP_KEYS` and `CONFIG_CHANNEL_KEYS` — if you see keys the script didn't report on (e.g. a channel name not in CHANNELS, or a top-level section like `integrations` or `plugins`), read that section of the config directly with the Read tool. Also check `STATE_DIR_CONTENTS` for directories the script doesn't scan (e.g. unexpected folders alongside `workspace/`, `agents/`, `cron/`).

**If STATUS=not_found:** Tell the user no OpenClawd installation was detected at the standard locations (`~/.openclawd`, `~/.clawdbot`). Ask if they have a custom path. If not, exit.

**If STATUS=found:** Present a human-readable summary:

- "I found your OpenClawd installation at `<STATE_DIR>`."
- Identity: name from IDENTITY.md (if found)
- Workspace files: which of SOUL.md, USER.md, MEMORY.md, IDENTITY.md exist
- Channels: list each, note which NanoClawd supports (whatsapp, telegram, slack, discord) and which it doesn't
...
- **Container isolation:** NanoClawd runs each agent in an isolated Linux container (Docker or Apple Container). OpenClawd runs everything in one process. This means stronger isolation but also means each group is its own sandbox.
- **Group-based memory:** In OpenClawd, all groups under one agent share the same SOUL.md, MEMORY.md, and IDENTITY.md. In NanoClawd, each group has its own filesystem and CLAUDE.md. Shared state goes in `groups/global/CLAUDE.md` (mounted read-only into all non-main containers).
- **Channel skills:** In OpenClawd, channels are configured in `openclawd.json`. In NanoClawd, channels are installed as code via skills (`/add-telegram`, `/add-whatsapp`, etc.) and configured through `.env` variables.
- **Simpler config:** NanoClawd has no config file — behavior is in the code and `CLAUDE.md` files. Credentials live in `.env` or the OneCLI vault.
...
**OpenClawd model:** All groups routed to the same agent share one workspace — the same SOUL.md, MEMORY.md, IDENTITY.md, and tools. When you talk to the bot in your family chat or your work chat, it's the same agent with the same personality and memory. Only the session (conversation history) is separate per group.

**NanoClawd model:** Each group is a completely separate agent running in its own Linux container. Separate filesystem, separate memory, separate CLAUDE.md. The bot in your family chat and your work chat are different agents that don't know about each other — unless you explicitly share state via `groups/global/CLAUDE.md`, which is mounted read-only into all non-main containers.
...
AskUserQuestion: "In OpenClawd, your groups shared the same personality and memory. In NanoClawd, each group is a fully separate agent. How would you like to handle this?"
...
IDENTITY_NAME from discovery gives the OpenClawd name. Ask the user: "Your OpenClawd assistant was named `<IDENTITY_NAME>`. Want to keep this name in NanoClawd?" If they want a different name, ask what it should be. If IDENTITY_NAME was empty, ask them to choose a name (default: "Andy").
...
The discovery script provides detected groups in the GROUPS field (format: `channel:id(name)=>nanoclawd_jid`). These are extracted from OpenClawd's session store and channel config.

For each group the user wants to bring over, pre-register it:

```bash
pnpm exec tsx setup/index.ts --step register -- --jid "<nanoclawd_jid>" --name "<group_name>" --folder "<channel>_<slug>" --trigger "@<confirmed_name>" --channel <channel> --assistant-name "<confirmed_name>"
```
...
Register groups from all channels — including channels NanoClawd doesn't yet support (signal, matrix, etc.). The registration stores the JID and metadata in the database, ready for when that channel is added later. Groups won't receive messages until their channel code is installed, but the registration, group folder, and CLAUDE.md will be ready.

## Phase 2: Settings from Config

Before identity/memory, extract settings from `openclawd.json` that map directly to NanoClawd setup. Read the config file with the Read tool (`<STATE_DIR>/openclawd.json` or `clawdbot.json`).

### Timezone

Check `agents.defaults.userTimezone` in the config. If present and it's a valid IANA timezone (e.g. `America/New_York`, `Asia/Jerusalem`), write it to `.env` as `TZ=<timezone>`. NanoClawd's setup step 2a reads `TZ` from `.env` (`src/config.ts:84-97`) and will skip the autodetection prompt.

### Anthropic Credentials

Check for Anthropic API keys or tokens in OpenClawd's auth system. OpenClawd stores credentials in `<STATE_DIR>/auth-profiles.json` or `<STATE_DIR>/agents/main/agent/auth-profiles.json` with this structure:
...
If found, offer to save to `.env`. This pre-fills the NanoClawd setup credential step (step 4) so the user doesn't need to re-enter it. Use the same masking approach — show first 4 + last 4 characters, write the full value directly.
...
Read the channel configs for access control settings. OpenClawd stores these per-channel:
...
NanoClawd uses `~/.config/nanoclawd/sender-allowlist.json` with this format:
```json
{
...
If OpenClawd had allowlists configured, show the user what was set and offer to create the NanoClawd equivalent. Map:
...
Check `agents.defaults.timeoutSeconds` in the config. This is maximum total agent runtime (wall-clock). NanoClawd's equivalent is `CONTAINER_TIMEOUT` (env var, default 30 min), also configurable per-group via `containerConfig.timeout`. Note: NanoClawd also has a separate `IDLE_TIMEOUT` (max time without output) which resets on activity — OpenClawd has no equivalent.

If the OpenClawd value differs significantly from 30 minutes, note it for the user. They can set `CONTAINER_TIMEOUT=<ms>` in `.env` after setup.
...
The STATE_DIR from discovery tells you where OpenClawd lives. Look for workspace files at `<STATE_DIR>/workspace/`. If AGENT_COUNT > 1, also check `<STATE_DIR>/agents/*/workspace/` and ask which agent to migrate.
...
Show the contents and discuss what's worth keeping. Some memory entries may be stale or OpenClawd-specific. Create `groups/main/memories.md` for relevant items. Add a reference in CLAUDE.md.

### Daily memory files (`workspace/memory/*.md`)

If DAILY_MEMORY_FILES > 0 in the discovery output, OpenClawd accumulated dated memory files (e.g. `2024-01-01.md`). These contain observations, facts, and context gathered over time.

AskUserQuestion: "You have N daily memory files from OpenClawd. How would you like to handle them?"
...
If SKILL_COUNT > 0 in discovery, OpenClawd had custom skills. The SKILL.md format is a shared standard — skills are directly portable.
...
If CONFIG_PLUGIN_COUNT > 0 in discovery, OpenClawd had installed plugins/skills with API keys (e.g. `plugins.entries.brave`, `skills.entries.openai-whisper-api`). These are functional tools the agent had access to.

For each detected plugin, present the name to the user and discuss whether to set it up in NanoClawd. Read the OpenClawd config section to understand what it is, then:

1. **If NanoClawd has a matching skill** — check the available NanoClawd skills list for an equivalent (e.g. `/add-voice-transcription` for whisper). If found, save the API key to `.env` and invoke that skill.

2. **If the OpenClawd plugin was an MCP server** — read its config to find the exact package name and command. Install the same MCP server (e.g. `pnpm dlx <exact-package-from-config>`). Don't search for or guess at MCP packages — only install what was explicitly configured.

3. **If the OpenClawd plugin was a CLI tool** — read the config to identify the exact tool. If it's an npm package, add it to the container's Dockerfile. Add a note to the group's CLAUDE.md that the tool is available and how to invoke it.
...
- TOOLS.md: NanoClawd agents have their own tool discovery; this doesn't transfer
- HEARTBEAT.md: NanoClawd uses scheduled tasks instead
- BOOTSTRAP.md: NanoClawd uses CLAUDE.md and container skills instead
- AGENTS.md: Already covered in the Phase 1 groups discussion

## Phase 4: Channel Credentials

For each channel found in the discovery results, handle it based on NanoClawd support:

### Supported channels (whatsapp, telegram, slack, discord)

Run the credential extraction script with `--write-env .env` so it writes credentials directly to NanoClawd's `.env` file. The script never emits raw credential values to stdout — only masked versions.
...
The script writes the credential directly to `.env` using the correct NanoClawd variable name (e.g. `TELEGRAM_BOT_TOKEN`). Check the status block for `WRITTEN_TO` and `WRITTEN_COUNT` to confirm.
...
Explain briefly: "NanoClawd doesn't have a `<channel>` integration yet, but channels are added over time via skills. Any groups from this channel were already registered in Phase 1 — they'll activate when the channel is added."
...
If jobs exist, read `${CLAUDE_SKILL_DIR}/MIGRATE_CRONS.md` for the full OpenClawd cron format, NanoClawd table schema, field mapping, and SQL insert template. Follow those instructions for each job.

## Phase 6: Webhooks, MCP, and Other Config

Read relevant sections from `<STATE_DIR>/openclawd.json` directly with the Read tool. This phase is fully conversational.

### MCP Servers

If MCP_SERVERS was non-empty in discovery, these can be ported. Claude Code supports MCP servers natively. Read the OpenClawd config's `mcp.servers` section to get each server's details (`command`, `args`, `env`, `url`).

MCP servers in NanoClawd are registered in the agent-runner source code. Before editing, grep for `mcpServers` in `container/agent-runner/src/` to find the current location — it's expected to be in `index.ts` in the `query()` options, but may have moved. For each OpenClawd MCP server the user wants to bring over:
...
- These don't map directly but NanoClawd can be customized to support them
...
- **Exec approvals / command allowlist:** NanoClawd uses container isolation instead — the agent runs with `--dangerously-skip-permissions` inside a sandboxed container
- **Human delay:** Not applicable in NanoClawd's container model
- **Compaction:** Handled by Claude Code SDK automatically
- **TTS:** Not built into NanoClawd
- **Model configuration:** NanoClawd uses whatever Anthropic model the credential provides access to
...
- Sender allowlists → `~/.config/nanoclawd/sender-allowlist.json`
...
- OpenClawd skills → copied to `container/skills/`
...
- OpenClawd-specific features (exec approvals, human delay, TTS, model config, session reset policies, etc.)

**Discussed and deferred:**
- List any customizations agreed on but not yet implemented

Remind: "Run `/setup` next to complete your NanoClawd installation. Channel credentials are already prepared in `.env`. When setup asks which channels to enable, select the ones we configured."

## Troubleshooting

**Config parse error:** If `openclawd.json` fails to parse, it may use JSON5 features the parser doesn't handle. Ask the user to check the file for unusual syntax. As a fallback, the agent can read the file directly and work with it manually.
...
**Multi-agent complexity:** If the user had many agents with different configs, focus on the primary/default agent first. Additional agents can be set up as separate NanoClawd groups later.
