# Migrating OpenClawd Cron Jobs to NanoClawd Scheduled Tasks
...
## OpenClawd Cron Job Format

Source: `<STATE_DIR>/cron/jobs.json` (from `src/cron/types.ts`). If the file format doesn't match what's described below, read the actual file and adapt — OpenClawd may have changed the schema.
...
## NanoClawd `scheduled_tasks` Table
...
- `delivery.mode:"webhook"` — NanoClawd has no webhook delivery. Discuss with the user: this could be implemented as a task `script` that runs `curl` to hit the webhook endpoint.
- `failureAlert` — NanoClawd has no failure alert system. Note this to the user.
- `wakeMode` — NanoClawd tasks always wake the agent immediately.
- `payload.model`, `payload.thinking`, `payload.timeoutSeconds` — NanoClawd doesn't support per-task model/thinking config. These are handled by the SDK.
- `deleteAfterRun` — NanoClawd `"once"` tasks are marked `"completed"` after running, not deleted.
...
If groups haven't been registered yet (database doesn't exist), save the task details to `groups/main/openclawd-migration-tasks.md` with the exact SQL payloads, and tell the user: "These tasks will be created after `/setup` registers your groups."
