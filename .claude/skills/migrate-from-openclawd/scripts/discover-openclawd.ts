/**
 * Discover an existing OpenClawd installation and emit a structured summary.
 *
...
function extractIdentityName(stateDir: string, workspaceDir: string | null): string {
  ...
  try {
    ...
    // IDENTITY.md uses key:value format, e.g. "name: Clawd"
...
/**
 * Map OpenClawd session key channel:kind:id to NanoClawd JID format.
 * OpenClawd keys: "whatsapp:group:120...@g.us", "telegram:group:-10012345"
 * NanoClawd JIDs: "120...@g.us", "tg:-10012345", "dc:12345", "slack:C12345"
 */
function toNanoClawdJid(channel: string, id: string): string {
  switch (channel) {
    case 'whatsapp':
...
function main(): void {
  ...
  // Format groups as "channel:id(name)" — also include NanoClawd JID mapping
  const groupList = groups
    .map(
      (g) =>
        `${g.channel}:${g.id}(${g.name})=>${toNanoClawdJid(g.channel, g.id)}`,
    )
    .join('|');
