/**
 * Extract a channel credential from an OpenClawd configuration and write it
 * directly to the NanoClawd .env file.
 *
 * Usage: pnpm exec tsx .claude/skills/migrate-from-openclawd/scripts/extract-channel-credentials.ts \
 *          --channel telegram --state-dir ~/.openclawd --write-env .env
 *
 * Handles OpenClawd SecretRef formats:
...
interface ChannelCredentialSpec {
  ...
  fields: string[];
  // Corresponding NanoClawd env var names
  envVars: string[];
}
...
function parseArgs(): { channel: string; stateDir: string; writeEnv: string } {
  ...
  if (!stateDir) {
    ...
    if (fs.existsSync(path.join(home, '.openclawd'))) {
      stateDir = path.join(home, '.openclawd');
    } else if (fs.existsSync(path.join(home, '.clawdbot'))) {
      stateDir = path.join(home, '.clawdbot');
    } else {
      console.error(
        'No OpenClawd directory found. Use --state-dir to specify.',
      );
      process.exit(1);
    }
  }
...
function main(): void {
  ...
  // OpenClawd stores Baileys auth at <stateDir>/credentials/whatsapp/<accountId>/
  // using useMultiFileAuthState (same as NanoClawd). The files are directly compatible.
  if (channel === 'whatsapp') {
    const authPaths = [
  ...
  if (!spec) {
    emitStatus({
      CHANNEL: channel,
      HAS_CREDENTIAL: false,
      NOTE: `Channel "${channel}" is not supported by NanoClawd. Supported: telegram, discord, slack, whatsapp.`,
    });
    return;
  }

  // Load OpenClawd config
  let config: Record<string, unknown> | null = null;
  for (const name of ['openclawd.json', 'clawdbot.json']) {
  ...
  if (!config) {
    emitStatus({
      CHANNEL: channel,
      HAS_CREDENTIAL: false,
      NOTE: 'Could not load openclawd.json',
    });
    return;
  }
