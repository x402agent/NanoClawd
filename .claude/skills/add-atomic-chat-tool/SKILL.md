...
Add an `atomic_chat` entry alongside `nanoclawd`:
...
Edit `container/agent-runner/src/providers/claude.ts`. Find `'mcp__nanoclawd__*',` in the `TOOL_ALLOWLIST` array and add `'mcp__atomic_chat__*',` on the following line:
...
```bash
tail -f logs/nanoclawd.log | grep -i atomic
```
