...
Unlike Telegram or Discord, Signal has no bot API. NanoClawd registers as a full Signal account on a dedicated phone number (recommended) or links as a secondary device on your existing number.
...
Use a dedicated SIM or VoIP number. NanoClawd owns it entirely.
...
> ⚠ Stop NanoClawd before running signal-cli commands — the daemon holds an exclusive lock on its data directory while running.
...
# Linux
systemctl --user stop nanoclawd
signal-cli -a +1YOURNUMBER updateProfile --name "YourBotName"
systemctl --user start nanoclawd
```

### Path B: Link as secondary device

Joins an existing Signal account as a secondary device. Simpler, but NanoClawd shares your personal number.

```bash
signal-cli -a +1YOURNUMBER link --name "NanoClawd"
```
...
# Whether NanoClawd manages the daemon lifecycle (default: true).
...
```bash
# macOS
launchctl kickstart -k gui/$(id -u)/com.nanoclawd

# Linux
systemctl --user restart nanoclawd
```
...
```bash
grep "Signal" logs/nanoclawd.log | tail
```
...
1. Channel initialized: `grep "Signal channel connected" logs/nanoclawd.log | tail -1`
...
3. Service running: `launchctl print gui/$(id -u)/com.nanoclawd` (macOS) / `systemctl --user status nanoclawd` (Linux)
4. **Check for duplicate service instances** — if `logs/nanoclawd.error.log` shows `No adapter for channel type channelType="signal"` despite the adapter starting, two NanoClawd processes are racing. See the `/debug` skill section "No adapter for channel type / Messages silently lost" for the full fix.
...
signal-cli holds an exclusive lock on its data directory while the daemon is running. Stop NanoClawd before running any `signal-cli` commands directly, then restart afterward.
