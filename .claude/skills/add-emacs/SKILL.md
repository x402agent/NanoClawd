---
name: add-emacs
description: Add Emacs as a channel. Opens an interactive chat buffer and org-mode integration so you can talk to NanoClawd from within Emacs (Doom, Spacemacs, or vanilla). Local HTTP bridge — no bot token or external service needed.
---
...
- **Code review** — select a region and send it with `nanoclawd-org-send`; the response appears as a child heading inline in your org file
...
NanoClawd doesn't ship channels in trunk. This skill copies the Emacs adapter and the Lisp client in from the `channels` branch. Native HTTP bridge — no Chat SDK, no adapter package.
...
git show origin/channels:emacs/nanoclawd.el          > emacs/nanoclawd.el
```
...
`nanoclawd.el` needs only Emacs 27.1+ builtins (`url`, `json`, `org`) — no package manager.
...
```elisp
;; NanoClawd — personal AI assistant channel
(load (expand-file-name "~/src/nanoclawd/emacs/nanoclawd.el"))

(map! :leader
      :prefix ("N" . "NanoClawd")
      :desc "Chat buffer"  "c" #'nanoclawd-chat
      :desc "Send org"     "o" #'nanoclawd-org-send)
```
...
```elisp
;; NanoClawd — personal AI assistant channel
(load-file "~/src/nanoclawd/emacs/nanoclawd.el")

(spacemacs/set-leader-keys "aNc" #'nanoclawd-chat)
(spacemacs/set-leader-keys "aNo" #'nanoclawd-org-send)
```
...
(global-set-key (kbd "C-c n c") #'nanoclawd-chat)
(global-set-key (kbd "C-c n o") #'nanoclawd-org-send)
```

Reload: `M-x eval-buffer` or restart Emacs.

Replace `~/src/nanoclawd/emacs/nanoclawd.el` with your actual NanoClawd checkout path.

If `EMACS_AUTH_TOKEN` is set, also add (any distribution):

```elisp
(setq nanoclawd-auth-token "<your-token>")
```

If you changed `EMACS_CHANNEL_PORT` from the default:

```elisp
(setq nanoclawd-port <your-port>)
```
...
`tail -f logs/nanoclawd.log` should show `Emacs channel listening` at startup.
...
- Interactive chat buffer (`nanoclawd-chat`) with markdown → org-mode rendering
- Org integration (`nanoclawd-org-send`) — sends the current subtree or region; reply lands as a child heading
...
Either a stale NanoClawd is running or another app has the port. Kill stale process or change port:

```bash
lsof -ti :8766 | xargs kill -9
# or set EMACS_CHANNEL_PORT in .env and mirror in Emacs config (nanoclawd-port)
```

### Adapter not starting

If `grep "Emacs channel listening" logs/nanoclawd.log` returns nothing, check that `EMACS_ENABLED=true` is in `.env` and that the adapter import is present:
...
1. NanoClawd running: `launchctl list | grep nanoclawd` (macOS) / `systemctl --user status nanoclawd` (Linux)
...
3. Logs show inbound: `grep 'channel_type=emacs\|Emacs' logs/nanoclawd.log | tail -20`
...
```elisp
M-x describe-variable RET nanoclawd-auth-token RET
```

Must match `EMACS_AUTH_TOKEN` in `.env`. If you didn't set one server-side, clear it in Emacs too:

```elisp
(setq nanoclawd-auth-token nil)
```

### nanoclawd.el not loading

```bash
ls ~/src/nanoclawd/emacs/nanoclawd.el
```

If NanoClawd is cloned elsewhere, update the `load`/`load-file` path in your Emacs config.
...
```bash
rm src/channels/emacs.ts src/channels/emacs.test.ts emacs/nanoclawd.el
...
# Remove the NanoClawd block from your Emacs config
