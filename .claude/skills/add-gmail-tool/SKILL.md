...
```bash
cat ~/.config/nanoclawd/mount-allowlist.json
```
...
Edit `container/agent-runner/src/providers/claude.ts`. Find `'mcp__nanoclawd__*',` in `TOOL_ALLOWLIST` and add `'mcp__gmail__*',` after it.
...
```bash
tail -100 logs/nanoclawd.log logs/nanoclawd.error.log | grep -iE 'gmail|mcp'
...
- `ENOENT: no such file or directory, open '/workspace/extra/.gmail-mcp/credentials.json'` → mount is missing. Check `~/.config/nanoclawd/mount-allowlist.json` includes a parent of `~/.gmail-mcp`.
...
4. `pnpm run build && ./container/build.sh && systemctl --user restart nanoclawd`.
...
- **Addresses:** [issue #1500](https://github.com/qwibitai/nanoclawd/issues/1500) (proxy Gmail/Calendar OAuth tokens through credential proxy) for the Gmail side.
- **Related PRs:** [#1810](https://github.com/qwibitai/nanoclawd/pull/1810) (pre-install Gmail/Notion MCP) overlaps on the "install the MCP server in the image" idea but bundles many unrelated changes; this skill is the focused OneCLI-native version.
