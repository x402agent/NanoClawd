...
Configure which host directories NanoClawd agent containers can access. The mount allowlist lives at `~/.config/nanoclawd/mount-allowlist.json`.

## Show Current Config

```bash
cat ~/.config/nanoclawd/mount-allowlist.json 2>/dev/null || echo "No mount allowlist configured"
```
