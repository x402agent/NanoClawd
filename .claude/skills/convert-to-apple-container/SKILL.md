...
This skill switches NanoClawd's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
```bash
git remote add upstream https://github.com/qwibitai/nanoclawd.git
```
...
```bash
grep -q 'nanoclawd proxy' /etc/pf.conf 2>/dev/null || echo '# nanoclawd proxy — block LAN access to credential proxy
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo nanoclawd-agent:latest "Container OK"
```
