...
This skill replaces the OneCLI gateway with NanoClawd's built-in credential proxy. Containers get credentials injected via a local HTTP proxy that reads from `.env` — no external services needed.
...
If neither check matches, you may be on an older version. Run `/update-nanoclawd` first, then retry.
...
```bash
tail -20 logs/nanoclawd.log | grep "Credential proxy"
```
