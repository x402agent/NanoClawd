...
Edit `container/agent-runner/src/providers/claude.ts`. Add `'mcp__calendar__*'` to `TOOL_ALLOWLIST` after `'mcp__nanoclawd__*'` (or after `'mcp__gmail__*'` if present).
...
```bash
docker ps -q --filter 'name=nanoclawd-v2-' | xargs -r docker kill
```
...
```bash
tail -100 logs/nanoclawd.log | grep -iE 'calendar|mcp'
```
