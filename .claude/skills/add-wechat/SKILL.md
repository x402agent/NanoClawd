...
- Node.js >= 20 (already required by NanoClawd)
...
- **Only one instance can use a given token at a time.** Don't run multiple NanoClawd instances pointing to the same `data/wechat/auth.json`.
...
- **post-install-wiring**: Use the `wire-dm.ts` helper (see the "Wire your first DM" section above) if running this skill standalone. If running as part of `bash nanoclawd.sh`, `init-first-agent.ts` handles wiring — just pass the `platform-id` and `admin-user-id` captured above.
