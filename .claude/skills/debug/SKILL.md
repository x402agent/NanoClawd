...
|-----|----------|---------|
| **Main app logs** | `logs/nanoclawd.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/nanoclawd.error.log` | Host-side errors |
...
**Symptom:** The bot stops replying. `logs/nanoclawd.error.log` shows repeated:
```
...
**Root cause: two NanoClawd service instances running simultaneously.**

When a second service instance (often `nanoclawd-v2-<id>.service` running alongside `nanoclawd.service`) is active with a stale binary, it has no channel adapters registered. Its delivery poll races against the working instance and wins — permanently marking outbound messages as delivered without ever sending them.

**Diagnosis:**
```bash
# Check for duplicate running instances
ps aux | grep 'nanoclawd/dist/index.js' | grep -v grep

# Check which services are active
systemctl --user list-units 'nanoclawd*' --all

# Confirm channel adapters registered by the current process
grep "Channel adapter started" logs/nanoclawd.log | tail -10
```
...
2. Stop and disable the stale duplicate service:
   ```bash
   systemctl --user stop nanoclawd.service   # or whichever is the old one
   systemctl --user disable nanoclawd.service
   ```
3. If the remaining service unit is missing `EnvironmentFile`, add it:
   ```bash
   # Edit the service unit — add this line under [Service]:
   # EnvironmentFile=/home/[user]/nanoclawd/.env
   systemctl --user daemon-reload
   systemctl --user restart nanoclawd-v2-<id>.service
   ```
4. Verify only one instance runs: `ps aux | grep nanoclawd/dist/index.js | grep -v grep`
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash nanoclawd-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash nanoclawd-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash nanoclawd-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash nanoclawd-agent:latest
```
...
# Also clear the session ID from NanoClawd's tracking (stored in SQLite)
pnpm exec tsx scripts/q.ts store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/nanoclawd.log | tail -5
...
```bash
echo "=== Checking NanoClawd Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo nanoclawd-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/nanoclawd.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
