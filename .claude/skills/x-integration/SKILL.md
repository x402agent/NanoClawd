---
name: x-integration
description: X (Twitter) integration for NanoClawd. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NanoClawd v1.0.0. Directory structure may change in future versions.
...
1. **NanoClawd is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep nanoclawd (macOS) or systemctl --user status nanoclawd (Linux)
```
...
| `NANOCLAWD_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nanoclawd.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NanoClawd, make the following modifications:
...
All paths below are relative to project root (`NANOCLAWD_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep nanoclawd  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status nanoclawd
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nanoclawd.log | tail -20

# Script errors
grep -i "error\|failed" logs/nanoclawd.log | tail -20
```
...
docker run nanoclawd-agent ls -la /app/src/skills/
```
