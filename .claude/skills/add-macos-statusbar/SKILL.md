...
description: Add a macOS menu bar status indicator for NanoClawd. Shows a bolt icon with a green/red dot indicating whether NanoClawd is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows NanoClawd's running status and lets the user
...
```bash
launchctl list | grep com.nanoclawd.statusbar
```
...
Create `~/Library/LaunchAgents/com.nanoclawd.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.nanoclawd.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.nanoclawd.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see NanoClawd's status and control the service.
>
> - **Green dot** — NanoClawd is running
> - **Red dot** — NanoClawd is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.nanoclawd.statusbar.plist
rm ~/Library/LaunchAgents/com.nanoclawd.statusbar.plist
rm dist/statusbar
```
