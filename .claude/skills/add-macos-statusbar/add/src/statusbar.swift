...
class StatusBarController: NSObject {
    ...
    private let plistPath = "\(NSHomeDirectory())/Library/LaunchAgents/com.nanoclawd.plist"

    /// Derive the NanoClawd project root from the binary location.
    ...
    private func setupStatusItem() {
        ...
        if let button = statusItem.button {
            if let image = NSImage(systemSymbolName: "bolt.fill", accessibilityDescription: "NanoClawd") {
                image.isTemplate = true
                button.image = image
            } else {
                button.title = "⚡"
            }
            button.toolTip = "NanoClawd"
        }
    }
    ...
    private func checkRunning() -> Bool {
        ...
        task.arguments = ["list", "com.nanoclawd"]
        let pipe = Pipe()
    ...
    private func updateMenu() {
        ...
        let label = isRunning ? "NanoClawd is running" : "NanoClawd is stopped"
    ...
    @objc private func restartService() {
        let uid = getuid()
        run("/bin/launchctl", ["kickstart", "-k", "gui/\(uid)/com.nanoclawd"])
        refresh(after: 3)
    }

    @objc private func viewLogs() {
        let logPath = "\(StatusBarController.projectRoot)/logs/nanoclawd.log"
