...
description: Finish migrating a NanoClawd v1 install into v2. Run after `bash migrate-v2.sh` completes. Seeds the owner, cleans up CLAUDE.local.md files, reconciles container configs, and helps port custom v1 code. Triggers on "migrate from v1", "finish migration", "v1 migration".
---
...
If yes, continue to Phase 1. If no, diagnose from `logs/nanoclawd.log` and re-test — don't proceed to deeper work on a broken router.
...
3. Restart the service if running so changes take effect:
   ```bash
   # Linux
   systemctl --user restart nanoclawd-v2-*
   # macOS
   launchctl kickstart -k gui/$(id -u)/com.nanoclawd-v2-*
   ```
