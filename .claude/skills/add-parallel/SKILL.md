...
Adds Parallel AI MCP integration to NanoClawd for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NanoClawd scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NanoClawd already set up and running
...
Add Parallel AI MCP servers after the nanoclawd server:
```typescript
const mcpServers: Record<string, any> = {
  nanoclawd: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__nanoclawd__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__nanoclawd__send_message
   4. Use mcp__nanoclawd__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nanoclawd-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep nanoclawd  # macOS
...
- Check task runs: `tail -f logs/nanoclawd.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.nanoclawd` (macOS) or `systemctl --user restart nanoclawd` (Linux)
