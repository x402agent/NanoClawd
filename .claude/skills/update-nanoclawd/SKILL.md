---
name: update-nanoclawd
description: Efficiently bring upstream NanoClawd updates into a customized install, with preview, selective cherry-pick, and low token usage.
---

# About

Your NanoClawd fork drifts from upstream as you customize it. This skill pulls upstream changes into your install without losing your modifications.

Run `/update-nanoclawd` in Claude Code.

## How it works

**Preflight**: checks for clean working tree (`git status --porcelain`). If `upstream` remote is missing, asks you for the URL (defaults to `https://github.com/qwibitai/nanoclawd.git`) and adds it. Detects the upstream branch name (`main` or `master`).
...
# Goal
Help a user with a customized NanoClawd install safely incorporate upstream changes without a fresh reinstall and without blowing tokens.
...
- Ask the user for the upstream repo URL (default: `https://github.com/qwibitai/nanoclawd.git`).
...
**Large drift check:** If the upstream commit count and age suggest the user has a lot of catching up to do, mention that `/migrate-nanoclawd` might be a better fit — it extracts customizations and reapplies them on clean upstream instead of merging. Offer it as an option but don't push.
...
- Restart the service to apply changes. Detect platform with `uname -s`:
  - **macOS (Darwin)**: `launchctl kickstart -k gui/$(id -u)/com.nanoclawd`
  - **Linux**: detect the service name with `systemctl --user list-units --type=service | grep nanoclawd | awk '{print $1}'`, then `systemctl --user restart <detected-name>`
  - **Manual** (no service found): restart `pnpm run dev`


## Diagnostics

1. Use the Read tool to read `.claude/skills/update-nanoclawd/diagnostics.md`.
