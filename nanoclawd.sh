#!/usr/bin/env bash
#
# NanoClawd — end-to-end setup entry point.
#
...
# NanoClawd splash — under-the-sea lobster mascot in truecolor braille,
...
# NanoClawd runs an agent container per session. Below this threshold the
...
if [ "$LOW_MEM" = true ]; then
  printf '  %s\n' "$(red 'Warning: this machine likely cannot run NanoClawd.')"
  printf '  %s\n' "$(dim 'NanoClawd recommends a 4 GB+ RAM machine. Below this, the host + agent')"
...
# NanoClawd is known to not run reliably on Google Compute Engine instances.
...
if [ "$(uname -s)" = "Linux" ] \
  ...
  printf '  %s\n' "$(dim 'Google blocks sudo commands, so NanoClawd is unlikely to run successfully on this VM.')"
  printf '  %s\n\n' "$(dim 'If you want to run NanoClawd successfully, switch to a different provider (Hetzner, Hostinger, exe.dev and others..).')"
...
if [ "$(uname -s)" = "Linux" ] && [ "$(id -u)" -eq 0 ]; then
  ...
  printf '  %s\n' \
    "$(dim "Running NanoClawd as root is not recommended. It can cause permission")"
  printf '  %s\n\n' \
    "$(dim "issues with containers, services, and file ownership.")"
  printf '  %s\n' "$(bold '1)') $(dim 'Show me instructions for creating a new Linux user')"
  printf '  %s\n\n' "$(bold '2)') $(dim 'Continue setting up NanoClawd as root user (not recommended)')"
  ...
  case "${ROOT_ANS:-1}" in
    ...
    *)
      ...
      printf '  %s\n' "$(dim '1. Create a new user:           adduser nanoclawd')"
      printf '  %s\n' "$(dim '2. Add to sudo group:           usermod -aG sudo nanoclawd')"
      printf '  %s\n' "$(dim '3. Enable passwordless sudo:    echo "nanoclawd ALL=(ALL) NOPASSWD:ALL" | tee /etc/sudoers.d/nanoclawd')"
      printf '  %s\n' "$(dim '4. Log out:                     exit')"
      printf '  %s\n' "$(dim '5. Log back in as the new user: ssh nanoclawd@your-server')"
      printf '  %s\n' "$(dim '6. Clone the repo:              git clone https://github.com/qwibitai/nanoclawd.git && cd nanoclawd')"
      printf '  %s\n\n' "$(dim '7. Re-run setup:               bash nanoclawd.sh')"
      exit 1
      ;;
  esac
fi
...
if [ "$(uname -s)" = "Darwin" ] && ! command -v brew >/dev/null 2>&1; then
  printf '  %s\n' \
    "$(dim "Homebrew isn't installed. NanoClawd uses it to install Node and Docker on your Mac.")"
  printf '  %s\n\n' \
  ...
  case "${BREW_ANS:-Y}" in
    [Yy]*|'')
      ...
      if ! command -v brew >/dev/null 2>&1; then
        ...
        printf '  %s\n\n' \
          "$(dim 'Install manually from https://brew.sh and re-run: bash nanoclawd.sh')"
        exit 1
      fi
      printf '\n'
      ;;
    *)
      printf '\n  %s\n\n' \
        "$(dim 'NanoClawd needs Homebrew. Install it from https://brew.sh and re-run.')"
      exit 1
      ;;
  esac
fi
...
BOOTSTRAP_EXIT_FILE=$(mktemp -t nanoclawd-bootstrap-exit.XXXXXX)
(
...
# --silent suppresses pnpm's `> nanoclawd@2.0.0 setup:auto / > tsx setup/auto.ts`
